...
var Register = React.createClass({
	...
	render: function() {
		return (
			<div>
				<Header title={this.props.forget && '重置密码' || '注册新账号'} right={null} />
					...
					<form className="u-registerform u-userform">
						...
						<div className="u-inputline f-clearfix">
							...
							<div className="u-buttonc f-fl">
								<a className="u-button" type="button" onClick={this.getCode}>{this.state.s && ('(' + this.state.s + 's)后可以重新发送') || '获取验证码'}</a>
							</div>
						</div>
						<div className="u-inputline">
						...
						<div className="u-inputline">
							<a className="u-button" onClick={this.handleSubmit}>完成</a>
						</div>
					</form>
				</div>
			</div>
